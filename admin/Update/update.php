...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $con->prepare("UPDATE user_cred SET
        first_name=?, last_name=?, email=?, age=?, phone=?, location=?, school=?, grade=?, login_type=?, st_mfis=?, gender=?, learning_type=?, ticket_type=?,ticket_sub_type=?,
        isaccepted=?, ip_address=?, browser=?, operating_system=?, submit_date=?, ticket_id=?, 
        enter_status=?, dinner_status=?, breakfast_status=? WHERE id=?");

    $stmt->bind_param("sssissssssssssssssssssi",  $newFirstName, $newLastName, $newEmail, $newAge, $newPhone, $newLocation, 
            $newSchool, $newGrade, $newLoginType, $newFromMFIS, $newGender, $newLearning_type, $newticket_type, $newticket_sub_type,
            $isAccepted, $ipAddress, $browser, $operatingSystem, $submitDate, $ticketId, 
            $enterStatus, $dinnerStatus, $breakfastStatus, $userId);
